...
void usbd_8dev_send_cmd_rsp(uint8_t error) {
    ...
    if (usbd_8dev_transmit_cmd_packet(&hUsbDeviceFS)) {
        error_handler();
    }
    if (usbd_8dev_receive_cmd_packet(&hUsbDeviceFS)) {
        error_handler();
    }
}
...
void usbd_8dev_transmit_can_frame() {
    ...
    if (RxHeader.IDE == CAN_ID_EXT) {
        ...
        buf_datatx.id = __builtin_bswap32(RxHeader.ExtId & 0x1ffffff);
    } else {
        buf_datatx.id = __builtin_bswap32(RxHeader.StdId & 0x00007ff);
    }
    if (RxHeader.RTR) buf_datatx.flags |= USB_8DEV_RTR;
    buf_datatx.dlc = RxHeader.DLC;
    memcpy(buf_datatx.data, RxData, RxHeader.DLC);
    ...
    if (usbd_8dev_transmit_data_packet(&hUsbDeviceFS)) {
        //error_handler();
    }
}
...
void usbd_8dev_receive() {
    if (usbd_8dev_receive_data_packet(&hUsbDeviceFS)) {
        error_handler();
    }
}

static uint8_t usbd_8dev_itf_init(void) {
    usbd_8dev_set_cmd_txbuf(&hUsbDeviceFS, (uint8_t*) &buf_cmdtx, sizeof(Msg_CmdTypeDef));
    usbd_8dev_set_cmd_rxbuf(&hUsbDeviceFS, (uint8_t*)  &buf_cmdrx);
    usbd_8dev_set_data_txbuf(&hUsbDeviceFS, (uint8_t*)  &buf_datatx, sizeof(Msg_TxTypeDef));
    usbd_8dev_set_data_rxbuf(&hUsbDeviceFS, (uint8_t*)  &buf_datarx);
    return USBD_OK;
}
    ...
    if (*len == sizeof(buf_cmdrx) && buf_cmdrx.start == USB_8DEV_CMD_START && buf_cmdrx.end == USB_8DEV_CMD_END) {
        ...
        switch (buf_cmdrx.command) {
            ...
            case USB_8DEV_OPEN:
                ...
                can_bittiming.brp = SystemCoreClock * ((buf_cmdrx.data[3] << 8) | buf_cmdrx.data[4]) / 32000000;
...
static uint8_t usbd_8dev_itf_rcv_data(uint8_t* buf, uint8_t *len) {
    ...
    if (*len == sizeof(buf_datarx) && buf_datarx.start == USB_8DEV_DATA_START && buf_datarx.end == USB_8DEV_DATA_END) {
        ...
        if (buf_datarx.flags & USB_8DEV_EXTID) {
            ...
            TxHeader.ExtId = __builtin_bswap32(buf_datarx.id) & 0x1fffffff;
        } else { // Base frame format
        	TxHeader.IDE = CAN_ID_STD;
        	TxHeader.StdId = __builtin_bswap32(buf_datarx.id) & 0x00007ff;
        }
        TxHeader.DLC = buf_datarx.dlc;
        memcpy(TxData, buf_datarx.data, buf_datarx.dlc);
        requests |= REQ_CAN_TX;
    } else {
        error_handler();
    }
